...
import { getLatestSubmissionMeta, getProcessingData, getSuppliers } from '../services/dataService'
...
const CurrentStockModal: React.FC<CurrentStockModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    setPdfUrl(null)
    setPdfName(null)
    Promise.all([getProcessingData(), getLatestSubmissionMeta(), getSuppliers()])
      .then(([proc, meta, sups]) => {
        setItems(proc)
        setLatest(meta)
        setSuppliers(sups)
  ...
  const handleGeneratePdf = async () => {
    try {
      setLoading(true)
      const { blob, fileName } = await generateCurrentStockPdf(sortedRows, latest, suppliers)
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50">
      <div className="bg-white text-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[92vh] flex flex-col">
        <header className="flex items-center justify-between p-4 border-b border-gray-200">
          <h2 className="text-xl font-bold text-[#B6432B]">Current Stock Report</h2>
        ...
        <main className="p-4 space-y-4 overflow-y-auto">
          {latest && (
            <div className="text-sm text-gray-700">
              <span className="font-semibold">Terakhir diperbarui:</span> {latest.timestamp}
              {latest.staff ? (<span> • <span className="font-semibold">Oleh:</span> {latest.staff}</span>) : null}
            </div>
          )}
          ...
          {!loading && !error && (
            <>
              <div className="flex flex-wrap gap-4 items-center text-sm text-gray-700">
                <span><span className="font-semibold">Bahaya:</span> {dangerCount}</span>
                <span><span className="font-semibold">Low:</span> {lowCount}</span>
              </div>
                  ...
                  <tbody>
                    {sortedRows.map((r, idx) => (
                      <tr key={idx} className={idx % 2 ? 'bg-gray-50' : ''}>
                        ...
                        <td className="px-3 py-2 align-top">{r.condition === '-' ? 'normal' : r.condition}</td>
              ...
              {pdfUrl && (
                <div className="text-sm text-green-700">PDF generated. Use the button below to download.</div>
              )}
            </>
          )}
        </main>
        ...
        <footer className="p-4 bg-gray-50 border-t border-gray-200 rounded-b-2xl flex flex-col sm:flex-row gap-3 justify-end">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {pdfUrl ? (
              <a href={pdfUrl} download={pdfName || 'current-stock.pdf'} className="px-6 py-2 bg-white border border-gray-300 text-gray-800 rounded-md hover:bg-gray-100 font-semibold text-center">Download PDF Report</a>
            ) : (
              <button onClick={handleGeneratePdf} disabled={loading || !!error} className="px-6 py-2 bg-[#B6432B] text-white rounded-md hover:bg-[#a03a25] font-semibold disabled:bg-gray-400 disabled:cursor-not-allowed">Generate PDF Report</button>
            )}

            <button onClick={() => setContactOpen(true)} className="px-6 py-2 bg-[#25D366] text-white rounded-md hover:bg-[#1EBE57] font-semibold flex items-center gap-2">
              <WhatsappIcon />
              Contact Supplier
            </button>
          </div>
      ...
      <SupplierContactModal
        ...
        itemsNeedingRestock={sortedRows.filter((i) => i.currentQty < i.parQty || i.condition !== '-')}
        suppliers={suppliers}
      />
    </div>
  )
